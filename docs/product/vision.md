...
- **Exercise banks** (v2.0): real, per-KC auto-gradeable exercises (multiple-choice, numeric-with-tolerance, fully-worked) grounded in actual course texts and problem-set conventions — not AI-generated on the fly. QuickQuiz prefers this seeded bank over AI generation, which means assessment works even with no AI provider configured; a KC's own detail page also gets a direct Exercises section for drilling outside the quiz flow.
