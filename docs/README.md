...
- **event-catalog.md** — Operational companion to the KLI doc: the canonical event catalog (every emitted domain event with emitter/payload/fold-read keys, plus the unwired vocabulary), expected lifecycles and orderings, idempotency guarantees, known defects, and the recorded-but-unimplemented behavioral analytics design (taxonomy, funnels, PostHog decision).
