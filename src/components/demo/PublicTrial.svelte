<script lang="ts">
  ...
  import { planDemo, previewDateLabel, type DemoCourse, type DemoPlan } from '../../lib/demo/planner';
  ...
  let previewPlan = $state<DemoPlan | null>(null);
  ...
  function demoCourseFromProposal(course: typeof draft.courses[number]): DemoCourse {
    return {
      ...
      branches: course.branches.map((branch) => ({
        name: branch.name,
        kcs: branch.kcs.map((kc) => ({ name: kc.name, template_ref: kc.template_ref, prereq_refs: kc.prereq_refs })),
      })),
    };
  }

  function buildPreview(course: typeof draft.courses[number] | undefined): DemoPlan | null {
    if (!course) return null;
    return planDemo({
      course: demoCourseFromProposal(course),
      weeklyHours,
      timezone: draft.context?.timezone ?? 'UTC',
    });
  }

  function markScenario(id: DemoScenarioId) {
    const current = demoDraft.get();
    persist({ completed_scenarios: [...new Set([...current.completed_scenarios, id])] });
  }

  function sessionStepsFor(plan: DemoPlan | null): string[] {
    const title = plan?.recommendation?.title;
    if (!title) return [];
    return [
      `Recall ${title} without notes`,
      `Work one example or explanation for ${title}`,
      'Write down one question to carry into your next study session',
    ];
  }

  function previewDayLabel(timestamp: number): string {
    try {
      return new Intl.DateTimeFormat('en', { weekday: 'short', timeZone: draft.context?.timezone ?? 'UTC' }).format(timestamp);
    } catch {
  ...
  onMount(() => {
    ...
    if (initialMode === 'demo' && loaded.courses.length === 0) simulateAll();
      ...
      previewPlan = buildPreview(loaded.courses[0]);
  ...
  function runScenario() {
    previewPlan = buildPreview(activeCourse);
    markScenario(activeScenario.id);
  ...
  function chooseScenario(id: DemoScenarioId) {
    selectedScenario = id;
    previewedScenario = null;
    previewPlan = draft.completed_scenarios.includes(id) ? buildPreview(activeCourse) : null;
  }
    ...
    <section class="setup-wrap" aria-labelledby="setup-title">
      ...
      {:else if step === 2}
        <div class="setup-card">
          ...
          <fieldset><legend>Guidance</legend><div class="segments">{#each [['self_directed','Let me explore'],['balanced','Balanced'],['tell_me_next','Tell me next']] as option}<button type="button" class:chosen={guidance === option[0]} aria-pressed={guidance === option[0]} onclick={() => guidance = option[0] as typeof guidance}>{option[1]}</button>{/each}</div></fieldset>
          <fieldset><legend>Goal depth</legend><div class="segments">{#each [['keep_up','Keep up'],['understand','Understand'],['master','Master deeply']] as option}<button type="button" class:chosen={depth === option[0]} aria-pressed={depth === option[0]} onclick={() => depth = option[0] as typeof depth}>{option[1]}</button>{/each}</div></fieldset>
              ...
              <div class="course-results">
                {#if filteredCourses.length > 0}
                  {#each filteredCourses as course}
                    <button type="button" class:selected={activeCourse?.template_id === course.slug} aria-pressed={activeCourse?.template_id === course.slug} onclick={() => selectTemplate(course.slug)}>
                      <strong>{course.code}</strong><span>{course.title}</span><small>{course.kc_count} suggested concepts</small>
                    </button>
                  {/each}
                {:else}
                  <p class="no-results" role="status">No reviewed courses match “{courseQuery}”. Enter a course map on the right to continue.</p>
                {/if}
              </div>
            </section>
            <section>
      ...
      <div class="demo-intro">
        ...
        <div class="metrics">
          {#if draft.simulated}
            ...
            <span><strong>{draft.preferences.weekly_hours} h</strong> preview capacity</span>
          {:else}
            <span><strong>{previewPlan?.topicCount ?? activeCourse?.branches.reduce((sum, branch) => sum + branch.kcs.length, 0) ?? 0}</strong> supplied topics</span>
            <span><strong>{draft.preferences.weekly_hours} h</strong> weekly capacity</span>
          {/if}
        </div>
      </div>
      ...
      <div class="demo-grid">
        <section class="situation-card">
          ...
          {#if draft.completed_scenarios.includes(activeScenario.id)}
            <div class="payoff" role="status">
              <span>✓ Local preview updated</span>
              {#if previewPlan?.recommendation}
                <strong>{previewPlan.recommendation.title} is first.</strong>
                <small>{previewPlan.rationale}</small>
            {:else}
                <strong>No session was scheduled.</strong>
                <small>{previewPlan?.rationale ?? 'Add a course topic to build a preview.'}</small>
              {/if}
            </div>
          {:else}
        ...
        <section class:ready={draft.completed_scenarios.includes(activeScenario.id)} class="next-card" aria-live="polite">
          {#if draft.completed_scenarios.includes(activeScenario.id) && previewPlan?.recommendation}
            ...
            <div class="preview-schedule">
              <strong>Local preview schedule</strong>
              <ol>
                {#each previewPlan.scheduled.slice(0, 4) as session}
                  <li><span>{previewDateLabel(session.start, draft.context?.timezone ?? 'UTC')}</span><b>{session.title}</b></li>
                {/each}
              </ol>
              <small>{previewPlan.scheduled.length} session{previewPlan.scheduled.length === 1 ? '' : 's'} fit this capacity preview{previewPlan.unplaced.length > 0 ? ` · ${previewPlan.unplaced.length} remain unscheduled` : ''}. Nothing was added to a calendar.</small>
            </div>
            ...
            {/if}
          {:else if draft.completed_scenarios.includes(activeScenario.id)}
            <div class="next-placeholder">
              <span class="placeholder-number">2</span>
              <p class="eyebrow">Then · Preview capacity</p>
              <h2>No session fits yet.</h2>
              <p>{previewPlan?.rationale ?? 'Add a course topic to build a local preview.'}</p>
            </div>
          {:else}
            <div class="next-placeholder">
              <span class="placeholder-number">2</span>
        ...
        <section class="week-card">
          <p class="eyebrow">Capacity preview</p>
          {#if previewPlan?.scheduled.length}
            <div class="week-days" aria-label="Scheduled preview sessions">
              {#each previewPlan.scheduled.slice(0, 5) as session}
                <span class="scheduled-day busy"><small>{previewDayLabel(session.start)}</small><i></i><b>{session.minutes}m</b></span>
              {/each}
            </div>
            <p>{previewPlan.scheduled.length} session{previewPlan.scheduled.length === 1 ? '' : 's'} fit the supplied capacity{previewPlan.unplaced.length > 0 ? ` · ${previewPlan.unplaced.length} remain unscheduled` : ''}. Nothing was added to a calendar.</p>
          {:else if previewPlan}
            <p>No session fits the supplied capacity yet. {previewPlan.rationale}</p>
          {:else}
...
<style>
  ...
  .back-button { border:0;background:none;color:#685f72;cursor:pointer;font-weight:800;padding:0;margin-bottom:22px; }
  ...
  .field-grid,.mini-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;margin-top:24px}.field-stack{display:flex;flex-direction:column;gap:7px;font-size:13px;font-weight:900;color:#645b6c}.field-label{display:block;margin-top:12px}.no-results{margin:10px 0 0;padding:12px;border:1px dashed #cfc0d8;border-radius:12px;color:#645b6c;font-size:13px;line-height:1.45}label{display:flex;flex-direction:column;gap:7px;font-size:13px;font-weight:900;color:#645b6c}input,select,textarea{width:100%;border:1px solid #d8cfdf;border-radius:13px;background:#fff;padding:12px 13px;color:#281f35}input:focus,select:focus,textarea:focus{outline:3px solid rgba(139,63,208,.16);border-color:#8b3fd0}.slider{margin:26px 0}.slider strong{color:#281f35}.slider input{padding:0;accent-color:#8b3fd0}
  ...
  .situation-card h2 { font-size:clamp(28px,3.6vw,42px);max-width:14ch;overflow-wrap:anywhere; }
  ...
  .next-card h2 { margin-top:9px;font-size:clamp(25px,3vw,34px);max-width:14ch;overflow-wrap:anywhere; }
  ...
  .preview-schedule { margin-top:12px;padding:13px 14px;border:1px solid #e4dee8;border-radius:14px;background:#fbfafd; }
  .preview-schedule>strong { display:block;font-size:11px;text-transform:uppercase;letter-spacing:.08em;color:#69349b; }
  .preview-schedule ol { display:grid;gap:6px;margin:9px 0 8px;padding:0;list-style:none; }
  .preview-schedule li { display:flex;align-items:baseline;gap:8px;min-width:0;font-size:12px; }
  .preview-schedule li span { color:#81778d;flex:0 0 86px; }
  .preview-schedule li b { min-width:0;overflow-wrap:anywhere;color:#4d4257; }
  .preview-schedule>small { display:block;color:#81778d;line-height:1.45; }
  ...
  .week-days .scheduled-day b { color:#69349b;font-size:10px; }
  ...
  @media(max-width:1080px){
    ...
    .demo-header .mobile-reset{display:inline-flex;min-height:40px;padding:8px 13px;font-size:12px;align-items:center;justify-content:center;border:1px solid #d8cfdf;border-radius:999px;text-decoration:none;background:white;color:#503d61}
