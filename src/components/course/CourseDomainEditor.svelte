<script lang="ts">
  ...
  let { courseId, initialDraft } = $props<{ courseId: string; initialDraft: CourseDraftV2 }>();
  let draft = $state(structuredClone(initialDraft)); let saving = $state(false); let error = $state('');
  async function save() {
    saving = true; error = '';
    const result = await apiFetch<{ slug: string }>(`/api/v1/courses/${courseId}/domain`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(draft) }, 'Could not save this course');
    saving = false;
    if (result.ok) window.location.href = `/courses/${result.data.slug}/concepts`; else error = result.error;
  }
</script>
<CourseMapReview {draft} onchange={(next) => { draft = next; }} />
{#if error}<p role="alert">{error}</p>{/if}
<button class="save" disabled={saving} onclick={save}>{saving ? 'Saving…' : 'Save as revised course'}</button>
<p class="note">Saving keeps prior learning history in an archived revision.</p>
