...
- Node: as of 2026-08-16 the machine's node is v22.23.1 at `~/.local/bin` (nvm is GONE — `~/.nvm` no longer exists, so the old `export PATH="$HOME/.nvm/versions/node/v24.16.0/bin:$PATH"` prefix is a no-op; plain `node`/`npm`/`npx` work, but `astro` still needs `npx astro`). Playwright's old "global under Node 20" install died with nvm — install it in the session scratchpad (`npm init -y && npm i playwright`); browser binaries persist in `~/Library/Caches/ms-playwright`.
