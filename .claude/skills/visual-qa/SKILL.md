...
- **Gotchas**: a corrupted Vite deps cache (404 on `@astrojs_svelte_client` → no island hydrates, clicks do nothing; or SSR 500s citing missing `deps_ssr/` files) means `astro dev stop && rm -rf node_modules/.vite && astro dev --force`. **Root cause**: running `npm run build` — or `astro check` / `npm run check` — while a dev server is live corrupts the shared `node_modules/.vite` cache; never run either mid-capture. Build/check first or after, then clean-restart the dev server. A page whose HTML truncates mid-stream with status 200 is an island SSR throw — check the served HTML's last tag, not just the status code. Reviewers judging tall full-page captures may misreport "tiny text"; verify suspicious findings by Reading the shot yourself. The dark rounded widget at bottom-center is Astro's dev toolbar, not app UI.
